...
rais = (
    spark.read
    .csv("s3://datalake-maxwell-igti-mba/raw-data/rais", inferSchema=True, header=True, sep=';', encoding="latin1")
)

enem = (
    spark.read
    .csv("s3://datalake-maxwell-igti-mba/raw-data/enem", inferSchema=True, header=True, sep=';', encoding="latin1")
)

print("Writing delta table...teste")
(
    rais
    .write.mode('overwrite')
    .format('delta')
    .partitionBy('year')
    .save("s3://datalake-mba-igti-tf/staging-zone/enem-t-emr")
)
